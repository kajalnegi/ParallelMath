...
def create_parallel_lines(draw, center, size, angle, fill, angle_type='co_interior', position=4):
...
def create_rotate90_parallel_lines(draw, center, size, angle, fill, angle_type='co_interior', position=4):
    """Draw a pair of parallel lines."""
...
def generate_samples_parallel_lines(output_dir, i, no_solution=False):

    
    df = pd.DataFrame(columns=['id', 'image_path', 'question', 'answer', ])
    angles = list(range(30,151,5))
    for pos, ang_type in position_angle_type.items():
        for ang_t in ang_type:
            for ang in angles:
                ...
                create_rotate90_parallel_lines(draw, center, size, ang, fill, angle_type=ang_t, position=pos)
                filename = f"{output_dir}{directory}/vertical.png"
                display(img)
                img.save(filename)
                draw, center, size, fill, img = define_canvas()
                create_parallel_lines(draw, center, size, ang, fill, angle_type=ang_t, position=pos)
    ...
    #df.to_csv(output_dir1+'input.csv', index=False)
    return df, i
